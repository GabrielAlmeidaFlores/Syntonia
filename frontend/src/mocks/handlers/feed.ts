 ...
 */
const getFeedHandler = http.get<never, never, FeedResponse>('/feed', async ({ request }) => {
  await delay(400);
  ...
  const limit = limitParam !== null ? Math.min(Number(limitParam), 10) : FEED_PAGE_SIZE;
 ...
 */
const getPostHandler = http.get<{ id: string }, never, Post | { error: string }>(
  '/post/:id',
  async ({ params }) => {
    await delay(200);
 ...
 */
const postFeedRequestHandler = http.post<never, GenerationRequestBody, GenerationResponse>(
  '/feed/request',
  async ({ request }) => {
    await delay(300);
    ...
    const requestIds = Array.from({ length: quantity }, (_, i) =>
      `req-mock-${Date.now().toString()}-${String(i)}`,
    );
...
export const feedHandlers = [getFeedHandler, getPostHandler, postFeedRequestHandler];
