 ...
 */
const savePostHandler = http.post<{ id: string }, never, SavePostResponse | { error: string }>(
  '/post/:id/save',
  async ({ params }) => {
    await delay(300);
 ...
 * The 300ms delay simulates a DynamoDB UpdateItem call.
 */
const unsavePostHandler = http.delete<{ id: string }, never, UnsavePostResponse | { error: string }>(
  '/post/:id/save',
  async ({ params }) => {
    await delay(300);
    ...
    const { id } = params;
    if (!savedIds.has(id)) {
      return HttpResponse.json({ error: 'Post not found in saved posts' }, { status: 404 });
    }

    savedIds.delete(id);
    runtimeSavedAt.delete(id);

    return HttpResponse.json({ message: 'Post unsaved. TTL restored to 30 days.' });
  },
);
...
export const savedHandlers = [savePostHandler, unsavePostHandler, getSavedPostsHandler];
