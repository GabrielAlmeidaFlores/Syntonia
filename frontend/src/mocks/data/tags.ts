...
export function mockExtractTags(description: string): Tag[] {
  ...
  const mappings: Array<[string[], Tag]> = [
    ...
    [['architecture', 'design pattern', 'microservice', 'system design'], 'Architecture'],
  ];
  ...
  if (extracted.length === 0) return ['Architecture', 'Performance', 'Security'];

  const first = extracted[0];
  if (extracted.length === 1 && first !== undefined) return [first, 'Architecture', 'Performance'];
